...
if __name__ == '__main__':
    ...
    with ProgressBar():
        ds.remesh('R64', RR, operator=gaussian_remesh, FGR=np.sqrt(12), exp='filtered-coarsegrained', compute=True)
    exp = ds['filtered-coarsegrained']

    dataset = xr.Dataset()
    for key in ['u', 'v', 'e', 'h', 'RV']:
        dataset[key] = exp.__getattribute__(key)
    dataset.to_netcdf(f'/home/ctrsp-2024/pp2681/notebooks/filtered-data/R64_{RR}_FGR-sqrt12-part1.nc')

    dataset = xr.Dataset()
    for key in ['ua', 'va', 'ea', 'ha']:
        dataset[key] = exp.__getattribute__(key)
    dataset.to_netcdf(f'/home/ctrsp-2024/pp2681/notebooks/filtered-data/R64_{RR}_FGR-sqrt12-part2.nc')
    ...
    dataset.to_netcdf(f'/home/ctrsp-2024/pp2681/notebooks/filtered-data/R64_{RR}_FGR-sqrt12-part3.nc')