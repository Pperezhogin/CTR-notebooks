...
class Experiment:
    ...
    def PE_joul(self, e):
        ...
        # This is because MOM6 output was changed in 2023
        try:
    ...
    def PE_ssh(self, e):
        hint = e.isel(zi=0)
        try:
            g = self.vert_grid.g.isel(Interface=0)
        except:
            g = self.vert_grid.g.isel(zl=0)
        return 0.5 * self.vert_grid.R.isel(zl=0)*g*(hint**2 * self.param.dxT * self.param.dyT).sum(dim=('xh','yh'))
